...
/**
 * 獲取前端動態配置 - 重構版本
 * 使用統一的 BaseResponse 處理和錯誤回復機制
 */
export const getFrontendConfig = async (): Promise<LocalFrontendConfig> => {
  try {
    const response = await apiClient.get<BaseResponse<LocalFrontendConfig>>('/frontend-config');
 ...
 * 當後端配置不可用時的後備配置
 */
export const getDefaultFrontendConfig = (): LocalFrontendConfig => {
  return {
    polling: {
      pollInterval: 2000,
