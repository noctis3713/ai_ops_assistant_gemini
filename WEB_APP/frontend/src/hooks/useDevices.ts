...
import { logError, logApiError } from '@/errors';
...
export const useDevices = () => {
  ...
  return useQuery<Device[], Error>({
    ...
    queryFn: async () => {
      try {
        try {
          logApiError('useDevices: 開始獲取設備列表', {});
        } catch {
          // 日誌記錄失敗時靜默處理
        }
        ...
        try {
          logApiError('useDevices: 成功獲取設備列表', { 
    ...
    retry: (failureCount, error) => {
      if (failureCount < cacheStrategy.retry) {
        try {
          logApiError(`useDevices: 重試第 ${failureCount + 1} 次`, { error: String(error) });
        } catch {
          // 日誌記錄失敗時靜默處理
        }
        return true;
