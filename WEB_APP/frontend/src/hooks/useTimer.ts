...
export const useTimer = () => {
   ...
   * 在指定時間內只執行最後一次調用
   */
  const debounce = useCallback((callback: TimerCallback, delay: number) => {
    let timerId: TimerId | null = null;
    
    return () => {
      if (timerId) {
        clearTimerSafe(timerId);
      }
      
      timerId = setTimeoutSafe(callback, delay);
    };
  }, [setTimeoutSafe, clearTimerSafe]);

  /**
   * 節流執行工具函數
   * 在指定時間內最多執行一次
   */
  const throttle = useCallback((callback: TimerCallback, delay: number) => {
    let lastExecuted = 0;
