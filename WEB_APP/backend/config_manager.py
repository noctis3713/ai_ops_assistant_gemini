...
class ConfigManager:
    ...
    def load_devices_config(self) -> Dict[str, Any]:
        ...
        try:
            if not config_path.exists():
                ...
                raise HTTPException(status_code=500, detail=f"設備配置檔案不存在: {config_path}")
            ...
            raise HTTPException(status_code=500, detail=f"設備配置檔案JSON格式錯誤: {str(e)}")
            ...
            raise HTTPException(status_code=500, detail=f"讀取設備配置檔案失敗: {str(e)}")
    ...
    def _validate_devices_config(self, config_data: Dict[str, Any]):
        ...
        if not isinstance(config_data, dict):
            ...
            raise HTTPException(status_code=500, detail="設備配置檔案格式錯誤：必須是JSON物件")
        
        if "devices" not in config_data:
            logger.error("設備配置檔案缺少 'devices' 欄位")
            raise HTTPException(status_code=500, detail="設備配置檔案格式錯誤：缺少 'devices' 欄位")
        
        if not isinstance(config_data["devices"], list):
            logger.error("設備配置檔案 'devices' 欄位必須是陣列")
            raise HTTPException(status_code=500, detail="設備配置檔案格式錯誤：'devices' 欄位必須是陣列")
        ...
        for i, device in enumerate(config_data["devices"]):
            if not isinstance(device, dict):
                ...
                raise HTTPException(status_code=500, detail=f"設備配置檔案格式錯誤：第 {i+1} 個設備必須是物件")
            
            for field in required_fields:
                if field not in device:
                    logger.error(f"設備配置檔案第 {i+1} 個設備缺少必要欄位: {field}")
                    raise HTTPException(status_code=500, detail=f"設備配置檔案格式錯誤：第 {i+1} 個設備缺少欄位 '{field}'")
    ...
    def load_groups_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"群組配置檔案JSON格式錯誤: {str(e)}")
        except PermissionError:
            ...
            raise HTTPException(status_code=500, detail=f"讀取群組配置檔案失敗: {str(e)}")
    ...
    def _validate_groups_config(self, config_data: Dict[str, Any]):
        ...
        if not isinstance(config_data, dict):
            ...
            raise HTTPException(status_code=500, detail="群組配置檔案格式錯誤：必須是JSON物件")
        ...
        if not isinstance(config_data["groups"], list):
            ...
            raise HTTPException(status_code=500, detail="群組配置檔案格式錯誤：'groups' 欄位必須是陣列")
        ...
        for i, group in enumerate(config_data["groups"]):
            if not isinstance(group, dict):
                ...
                raise HTTPException(status_code=500, detail=f"群組配置檔案格式錯誤：第 {i+1} 個群組必須是物件")
            
            for field in required_fields:
                if field not in group:
                    logger.error(f"群組配置檔案第 {i+1} 個群組缺少必要欄位: {field}")
                    raise HTTPException(status_code=500, detail=f"群組配置檔案格式錯誤：第 {i+1} 個群組缺少欄位 '{field}'")
        ...
        try:
            ...
            for device in config.get("devices", []):
                if device["ip"] == device_ip:
                    logger.debug(f"找到設備配置: {device_ip} -> {device.get('name', 'Unknown')}")
                    return device
    ...
    def get_device_name_safe(self, device_config: Optional[Dict[str, Any]], device_ip: str) -> str:
        """安全的設備名稱獲取函數
