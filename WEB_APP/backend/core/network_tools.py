...
class CommandValidator:
    ...
    def _get_validation_config(cls) -> Dict[str, Any]:
        ...
        if (
            ...
            or cls._config_last_loaded is None
            or current_time - cls._config_last_loaded > 30
        ):
    ...
    def validate_command(cls, command: str) -> Tuple[bool, Optional[str]]:
        ...
        allowed_prefixes = validation_config.get(
            "allowed_command_prefixes", cls.DEFAULT_ALLOWED_PREFIXES
        )
        dangerous_keywords = validation_config.get(
            "dangerous_keywords", cls.DEFAULT_DANGEROUS_KEYWORDS
        )
        ...
        if len(command) > max_length:
            return False, cls._generate_security_alert(
                command, f"指令長度超過 {max_length} 字元限制"
            )
        ...
        return False, cls._generate_security_alert(
            command, f"只允許 {allowed_prefixes_str} 開頭的指令"
        )
        ...
        for prefix in allowed_prefixes:
            if prefix == "show":
                ...
                command_examples.extend(
                    ["• show version", "• show interface", "• show environment"]
                )
            elif prefix == "ping":
...
class ConnectionPool:
    ...
    def get_connection(
        ...
        with self.lock:
            ...
            if (
                device_ip in self.connections
                and current_time - self.connection_times.get(device_ip, 0)
                < self.timeout
            ):
                try:
                    connection_age = current_time - self.connection_times.get(
                        device_ip, 0
                    )
                    if (
                        device_ip in self.connections
                        and connection_age > self.health_check_interval
                        and current_time - self.last_health_check
                        > self.health_check_interval
                    ):
                        self.connections[device_ip].send_command(
                            "show clock", read_timeout=5
                        )
    ...
    def cleanup_expired_connections(self):
        ...
        with self.lock:
            expired_ips = [
                ip
                for ip, connect_time in self.connection_times.items()
...
def _process_long_output(
    ...
    if len(output) > max_chars:
        logger.warning(
            f"輸出超過上限 ({max_chars} 字元): {command} (模式: {execution_mode})"
        )
        return (
            output[:max_chars]
    ...
    if global_ai_summary_enabled and enable_ai_summary and output_summarizer.llm:
        logger.info(
            f"使用 AI 摘要處理超長輸出: {command} (模式: {execution_mode}, 長度: {len(output)})"
        )
        return output_summarizer.summarize_output(command, output)
    else:
        # 使用截斷處理
        reason = (
            "設備指令模式" if execution_mode == "device" else "AI 摘要不可用或未啟用"
        )
        logger.info(
            f"截斷超長輸出: {command} (模式: {execution_mode}, 原因: {reason}, 長度: {len(output)})"
        )
        return (
            output[:ai_threshold]
...
def run_readonly_show_command(device_ip: str, command: str, device_config=None) -> str:
    ...
    if connection:
        try:
            ...
            processed_output = _process_long_output(
...
def _direct_connection_fallback(
    ...
    try:
        ...
        error_msg = (
            f"錯誤：無法連線到設備 {device_ip}，連線超時。請檢查IP位址和網路連線。"
        )
