...
def classify_error(error_message: str) -> Dict[str, Any]:
    ...
    # 安全相關錯誤
        ...
        "安全限制" in error_message
        or "不安全指令" in error_message
        or "安全警告" in error_message
    ):
        return {
    ...
    elif "memory" in error_lower and (
        "insufficient" in error_lower or "不足" in error_message
    ):
        return {
...
def unified_network_command_task(task: Task, command: str) -> Result:
    ...
    try:
        ...
        result = run_readonly_show_command(
            ...
            command=command,
            device_config=(
                type("DeviceConfig", (), device_config)() if device_config else None
            ),
        )
        ...
        if result.startswith("錯誤："):
            if any(
                ...
                for keyword in ["timeout", "connection", "連線"]
            ):
                ...
                retry_result = run_readonly_show_command(
                    ...
                    command=command,
                    device_config=(
                        type("DeviceConfig", (), device_config)()
                        if device_config
                        else None
                    ),
                )
            ...
            return Result(
                host=task.host, result=result, failed=True, exception=Exception(result)
            )
...
class NornirManager:
    ...
    def _initialize_nornir(self):
        ...
        try:
            ...
            inventory = self._build_inventory_from_dict(
            ...
            config = Config(
                ...
                logging={
                    ...
                    "level": "INFO",
                    "log_file": str(
                        Path(__file__).parent.parent / "logs" / "network.log"
                    ),
                },
            ...
            logger.info(
                f"Nornir 初始化成功，載入 {len(self.nr.inventory.hosts)} 台設備（使用記憶體字典，無檔案 I/O）"
            )
            network_logger.info(
                f"Nornir 初始化成功，載入 {len(self.nr.inventory.hosts)} 台設備（使用記憶體字典，無檔案 I/O）"
            )
    ...
    def _build_defaults_data(self) -> Dict[str, Any]:
        ...
        if not default_username or not default_password:
            first_device = (
                self.devices_config.get("devices", [{}])[0]
                if self.devices_config.get("devices")
                else {}
            )
    ...
    def _build_inventory_from_dict(
        self,
    ...
    def run_batch_command(
        self, command: str, device_ips: List[str] = None
    ) -> BatchResult:
        """對多台設備執行批次指令
        ...
        # 執行批次指令 - 使用統一的任務函式
        try:
            result = filtered_nr.run(task=unified_network_command_task, command=command)
            ...
            for host_name, task_result in result.items():
                ...
                if task_result.failed:
                    ...
                    logger.error(
                        f"設備 {host_name} ({device_ip}) 執行失敗: {task_result.exception}"
                    )
                else:
    ...
    def run_group_command(self, command: str, group_name: str) -> BatchResult:
        ...
        try:
            ...
            logger.info(
                f"對群組 '{group_name}' 執行指令，包含 {len(device_ips)} 台設備"
            )
...
def format_batch_result_for_ai(result: BatchResult) -> str:
    ...
    for device_ip, output in result.results.items():
        response_data["successful_results"].append(
            {"device_ip": device_ip, "output": output}
        )
...
def format_batch_result(result: BatchResult) -> str:
    ...
    if result.results:
        ...
        for device, device_result in list(result.results.items())[:3]:  # 只顯示前3台
            ...
            if "environment" in result.command.lower() and device_result:
                if any(
                    keyword in device_result.lower()
                    for keyword in ["temperature", "temp", "celsius", "°c", "degrees"]
                ):
                    output += "✅ 溫度資訊已成功獲取\n"
                elif (
                    "no output" in device_result.lower()
                    or len(device_result.strip()) < 10
                ):
    ...
    if result.failed_devices == 0:
        output += (
            f"✅ 【分析結論】所有 {result.successful_devices} 台設備指令執行成功！\n"
        )
