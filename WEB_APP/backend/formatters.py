...
def format_ai_results(devices: List[str], ai_result: str, execution_time: float = 2.0) -> Dict[str, Any]:
    """
    格式化 AI 查詢模式的執行結果
...
def format_command_results(batch_result) -> Dict[str, Any]:
    ...
    for device_ip, error_msg in batch_result.errors.items():
        ...
        # 格式化錯誤詳細資訊
        formatted_error = format_error_details(error_msg, batch_result.error_details.get(device_ip, {}))
    ...
    return {
        ...
        "summary": {
            ...
            # 包含快取統計（如果有）
            "cacheStats": _format_cache_stats(batch_result) if hasattr(batch_result, 'cache_hits') else None
        }
    }
...
def create_unified_api_response(results: List[Dict[str, Any]], 
