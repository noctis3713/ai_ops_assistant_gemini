            ...
            for key in usage_keys:
                if key in metadata:
                    ...
                    if isinstance(usage_info, dict):
                        ...
                        # Gemini 特定格式
                        input_tokens = usage_info.get("promptTokenCount", 
                                                    usage_info.get("prompt_token_count", 0))
                        output_tokens = usage_info.get("candidatesTokenCount",
...
class TokenLogger:
    ...
    def log_usage(self, task_id: str, provider: str, model: str, usage_data: Dict[str, Any], cost: float):
        """記錄 Token 使用量到日誌
    ...
    def display_summary(self, provider: str, total_tokens: int, cost: float, is_estimated: bool = False):
        """在控制台顯示 Token 使用量摘要
