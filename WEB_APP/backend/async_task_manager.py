...
class AsyncTaskManager:
        ...
        async with self._lock:
            for task_id, task in list(self.tasks.items()):
                ...
                if (task.is_finished() and 
                    (current_time - task.created_at) > self.task_ttl):
                    expired_tasks.append(task_id)
                    del self.tasks[task_id]
        
        if expired_tasks:
            self._stats["tasks_cleaned"] += len(expired_tasks)
            task_logger.info(f"已清理 {len(expired_tasks)} 個過期任務", extra={
    ...
    async def create_task(self, task_type: TaskType, params: Dict[str, Any]) -> AsyncTask:
        ...
        task_logger.info(f"已建立任務", extra={
            "task_id": task_id,
    ...
    async def list_tasks(self, status_filter: Optional[TaskStatus] = None, 
                        task_type_filter: Optional[TaskType] = None,
                        limit: Optional[int] = None) -> List[AsyncTask]:
        """
        列出任務，支援篩選
    ...
    async def update_task_status(self, task_id: str, status: TaskStatus):
        ...
        task_logger.debug(f"任務狀態已更新", extra={
            "task_id": task_id,
    ...
    async def update_progress(self, task_id: str, percentage: float = None, 
                            stage: str = None, **details):
        """
        更新任務進度
        ...
        async with self._lock:
            ...
            if task.status != TaskStatus.RUNNING:
                task_logger.warning(f"嘗試更新非運行狀態任務的進度: {task_id}, 狀態: {task.status.value}")
                return
        ...
        task_logger.debug(f"任務進度已更新", extra={
            "task_id": task_id,
    ...
    async def complete_task(self, task_id: str, result: Any):
        ...
        task_logger.info(f"任務已完成", extra={
            "task_id": task_id,
            "execution_time": task.execution_time if task else None,
    ...
    async def fail_task(self, task_id: str, error: str):
        ...
        task_logger.error(f"任務執行失敗", extra={
            "task_id": task_id,
            "error": error,
            "execution_time": task.execution_time if task else None
        })
    ...
    async def cancel_task(self, task_id: str, reason: str = "用戶取消"):
        ...
        async with self._lock:
            ...
            if task.is_finished():
                task_logger.warning(f"嘗試取消已完成的任務: {task_id}, 狀態: {task.status.value}")
                return False
        ...
        task_logger.info(f"任務已取消", extra={
            "task_id": task_id,
            "reason": reason
        })
        return True
...
async def create_and_run_task(task_type: TaskType, params: Dict[str, Any], 
                             worker_func: Callable) -> str:
    """
    建立任務並在背景執行
